...
LoadOrInstallLibraries(c("RODBC", "randomForest", "prodlim", "yaml", "devtools", "futile.logger"))
flog.appender(appender.file("estimates.log"), "quiet")
...
dbhandle <- GetDBHandle("devdenver");


# Query database to get features and combine into one frame

ticketcount <- sqlQuery(dbhandle,
  "SELECT BADGENUMBER, OFFICERNAME, ISSUEDATE, BEATNAME, count(*) AS TICKETCOUNT FROM
...
oms_session_feats <- sqlQuery(dbhandle,
  "SELECT O.OFFICERID AS BADGENUMBER, O.OFFICERNAME, CAST(O.DATETIME AS DATE) AS DATEBEAT,
    O.RECID AS SESSIONID, O.DATETIME, O.DATETIME2,
    D.TOTALLENGTH AS SESSIONLENGTH, D.PATROLLENGTH, D.SERVICELENGTH, D.OTHERLENGTH
  ...
  if ( combined_feats_GT_test$DATEBEAT[i] == Sys.Date() ) {
    ...
    if ( nrow(train) < 100 ) {
      ...
      for( j in 1:nrow(citReasonframe) ) {
        regexp <- paste("^", citReasonframe$feature[j], "$", sep="")
        actualValue <- combined_feats_GT[i, grep(regexp, names(combined_feats_GT))]
        loopReason <- paste(citReasonframe$feature[j], citReasonframe$percentMSE[j],
            citReasonframe$percentNodePurity[j], actualValue, sep=":")
        citReason <- paste(citReason, loopReason, sep=";")
      }
